...
def render_native_chart(xlsx_bytes: bytes, sheet_index: int = 3, chart_index: int = 1) -> bytes | None:
    """渲染 Excel 工作簿中指定子表的原生图表为 PNG 字节。

    两个模板的图表都位于第 3 个子表（图片 / 曲线图）。
    返回 PNG 字节；当前环境无法渲染时返回 None。
    """
    try:
    ...
    try:
        ...
        for progid in ("KET.Application", "Excel.Application"):
            ...
            try:
                ...
                logger.warning("%s 渲染图表预览失败：%s", progid, exc)
            finally:
                if app is not None:
                    try:
                        app.Quit()
