...
- 天数叠合曲线模板：负值修复（气量→0、油压/套压→前值×0.9/0.95）、滑动窗口固定值
  ...
  数据源兼容两种格式：原始单井数据自动拉齐，或直接上传已拉齐的数据
  （天数/平均套压/平均油压/平均日产气，按关键词 天/套/油/气 自动识别）；
...
well_chart/
  ...
  chart_render.py      # 原生 Excel 图表实时渲染（本地预览）
...
- v1.0.10：两个模板的“曲线图预览”实时显示下载结果中的原生图表
  （本机通过 WPS/Excel 渲染；服务器环境自动回退为近似预览）。
- v1.0.9：单井模板纵坐标刻度控制为 4~6 个节点，图例去掉单位。
- v1.0.8：天数叠合模板支持直接使用时间拉齐格式数据源；样式调整（红/蓝/橙新配色、
  纵坐标保留一位小数、图例无单位、纵轴标题竖排（从右到左）“压力（兆帕）”）。
