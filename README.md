...
「天数叠合曲线模板」已上线：把单井历史数据按生产天数拉齐（同一天多条记录取
平均），输出“时间拉齐数据 / 清洗日志 / 曲线图”三个子表，曲线图为原生可编辑
折线图（横坐标为生产天数，固定 6 个等距离刻度）。
...
- 自动清洗：负值替换为 NaN、油压/套压固定值段识别与线性插值修复、
  瞬时气量零值保留（真实关井）；
- 瞬时气量自动保留 4 位小数；
- 处理后的数据中，被插值修改的数据以浅黄色标出、无法修复的空值以浅红色标出；
- 图片子表中的图表默认带黑色边框；
...
- 输出 Excel 包含三个子表：
  ...
  - 「图片」：原生可编辑 Excel 图表（双击可编辑、修改数据后自动更新，不嵌入静态图片）；
- 天数叠合曲线模板：负值修复（气量→0、油压/套压→前值×0.9/0.95）、滑动窗口固定值
  检测（连续 10 点标准差<0.01 插值）、气量仪表故障修复（固定正值用均值替换）、
  按生产天数拉齐、原生 Excel 折线图（15×8 厘米、6 个天数刻度、无网格线、无图名）；
...
- v1.0.7：移除日期叠合曲线模板，仅保留单井生产曲线模板（天数叠合曲线模板待开发）。
- v1.0.6：各模板下载结果独立（文件名/标注带模板名）。
- v1.0.5：修复曲线丢失问题；纵坐标改为整数刻度。
- v1.0.4：日期叠合曲线模板开发。
- v1.0.3：瞬时气量保留 4 位小数；修改数据标黄；图表黑色边框。
- v1.0.2：图例加单位；三个子表结构。
- v1.0.1：多模板门户上线。
