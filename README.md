...
「日期叠合曲线模板」已上线（瞬时气量以面积图展示，其余与单井模板一致）；
「天数叠合曲线模板」即将上线（网页已预留入口）。
...
- v1.0.6：各模板下载结果独立（文件名/标注带模板名）；修复部署中间版本导致的模块不一致。
- v1.0.5：修复日期叠合曲线模板油压/套压曲线丢失问题；两个模板纵坐标改为整数刻度。
- v1.0.4：日期叠合曲线模板上线（瞬时气量为面积图，横坐标为真日期轴）。
